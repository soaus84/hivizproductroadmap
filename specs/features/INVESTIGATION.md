...
> **This is the canonical source for investigation prompt text, schemas, and pipeline logic — from the point an investigation is open through to FW Map® classification and the systemic cause bridge to the insight pipeline.** Note: the `investigation.assist` job that runs immediately after investigation creation is documented in `INCIDENT-CAPTURE.md` Stage 3, because it fires as part of the incident triage pipeline. This file covers everything from that point forward: investigator workflow, witness enquiry, systemic cause phase, toolbox narrative generation, and FW classification.

---

## What This Feature Is

An investigation is the structured human-and-AI process that follows a significant incident or an approved CriticalIncident. It begins when a CriticalIncident is approved (or triage creates a record directly for immediate investigations) and `investigation.assist` fires. It ends when the investigator closes it with confirmed framework fields and a sharing decision.

The investigation pipeline is the only sanctioned bridge between the incident pipeline and the insight pipeline. That bridge is Stage 3 — the systemic cause phase — where a completed investigation's findings can be entered as a CriticalInsight and enter the observation pipeline for broader action.

The feature has five stages:
...
Stage 3 — Systemic cause phase (optional)     (human-initiated — bridge to CRITICAL-INSIGHT.md)
Stage 4 — Toolbox narrative generation        investigation.generate_narrative   (async AI, on close)
Stage 5 — FW Map® classification              fw_classify (investigation path)   (async AI, on close)
```

Stage 1 is listed here for completeness but its prompt lives in `INCIDENT-CAPTURE.md` Stage 3. Stages 4 and 5 are the canonical prompts owned by this file.

---

## Global References Used

Investigation jobs in this file (`investigation.generate_narrative`, Stage 4; `fw_classify` investigation path, Stage 5) consume already-confirmed framework fields and classified energy/barrier values from the upstream incident — no independent classification happens here. Under Rule 1, taxonomy references still inject their `SUMMARY-REFERENCE` block at runtime to ground accurate framing. The Blueprint reaches the pipeline only at Stage 5 (`fw_classify`), which loads it under Rule 2. The `Role` column is diagnostic — see `HOW-TO-READ-THIS.md §Global Injection Rules`.
...
| FW Map® Blueprint | `globals/fw-map-blueprint.md` | Full Blueprint injected into `fw_classify` system prompt at runtime (Stage 5) under Rule 2 | Rule 2 — Full (Stage 5 only — via `fw-classify-job.md`) |
| Energy type taxonomy | `globals/energy-type-taxonomy.md` | Referenced in toolbox narrative context — value passes through from upstream classification | pass-through (Stage 4) |
...
- **Prompt lab** — loads Stage 4 (`investigation.generate_narrative`) and Stage 5 (`fw_classify` investigation path) from this file.
...
```
open        ← created on CriticalIncident approval or direct triage; investigation.assist fires immediately
closed      ← investigator confirms framework fields and signs off
escalated   ← systemic cause phase initiated (Stage 3); investigation remains closed but escalation noted
```
...
The investigator can also dispatch a mid-investigation cross-site enquiry if they suspect the incident condition exists at other sites (`trigger_source = investigation_mid`). See `ENQUIRY.md` for full enquiry pipeline.

---

## Stage 3 — Systemic Cause Phase (Optional)

**Human-initiated. The only sanctioned bridge from the incident pipeline to the insight pipeline.**

When an investigation's confirmed findings point to a systemic condition that likely exists beyond the incident or site — not adequately addressed by site-level corrective actions alone — the investigator or safety manager initiates the systemic cause phase. This creates a CriticalInsight via `trigger_source = 'external_investigation'` and enters it into the insight pipeline for broader action.

**This is not automatic.** The systemic cause phase is a deliberate human decision, not an algorithmic trigger. It requires the investigator or safety manager to judge that the root cause has organisation-level implications.

### When to initiate

The systemic cause phase is appropriate when:
- Root cause points to a shared management system, training gap, or equipment specification that applies across sites
- Contributing factors suggest a condition that other sites working the same work type are likely to share
- The investigation's FW Map® classification (Stage 5) returns a factor in the GUIDE or ENABLE domain — indicating management or system failure rather than individual action

It is not appropriate when:
- The investigation is contained to a single site incident with site-specific contributing factors
- `legal_hold = true` — systemic cause phase is blocked while legal hold is active
- The same systemic condition is already the subject of an active CriticalInsight in the insight pipeline

### How to initiate

```
POST /api/v1/critical-insights
{
  "trigger_source": "external_investigation",
  "trigger_id": "{{investigation_id}}",
  "work_type_id": "{{investigation.work_type_id}}",
  "generated_at_level": "{{recommended scope — region or above}}",
  "scope_ref_id": "{{relevant org entity id}}",
  "pattern_summary": "{{safety manager drafts — or pre-populated from root_cause + contributing_factors}}",
  "likely_systemic_cause": "{{from confirmed root_cause}}",
  "recommended_action": "{{from confirmed corrective_actions — highest-impact}}",
  "source_metadata": {
    "investigation_id": "{{uuid}}",
    "investigation_ref": "{{INV-ref}}",
    "systemic_cause_summary": "{{1-2 sentence summary of why systemic escalation is warranted}}"
  }
}
```

**On creation:** `external_investigation` CriticalInsights are auto-approved — `cleared_for_toolbox = true` is set immediately, `reviewed_by_id = creator`. The `fw_classify` job queues immediately. No AI draft step (Stage 1 of the insight pipeline is skipped). The safety manager's authored content is authoritative.

**Investigation record update:** When the systemic cause phase is initiated, `investigation.status` is set to `'escalated'` and `investigation.systemic_investigation_id` is set to the new CriticalInsight ID. This creates an auditable link between the two pipeline entities.

### What happens next

From this point, `CRITICAL-INSIGHT.md` governs. The CriticalInsight enters the insight pipeline at Stage 3 (FW Map® classification) and proceeds through Stage 4 (downstream dispatch): toolbox talk pool, enquiry generation, situational brief, CoP thread seed.

The investigation remains closed. The systemic cause phase is additive — it does not re-open the investigation or block any investigation-level outputs.

### Legal hold interaction

`investigation.legal_hold = true` blocks the systemic cause phase entirely. The `POST /api/v1/critical-insights` call is rejected at the API layer if the source investigation has `legal_hold = true`. This prevents investigation content from entering the sharing pipeline while legal hold is active, regardless of systemic significance.

---

## Stage 4 — Toolbox Narrative Generation
...
**Triggered:** Investigation closed + `cleared_for_sharing = true` + `legal_hold = false` — queues alongside Stage 4, runs in parallel
...
- If FW domain is GUIDE or ENABLE, the UI surfaces a prompt suggesting the systemic cause phase (Stage 3) — not automatic, investigator's decision
...
|---|---|---|
| Investigation opened | N-INV-ASSIGN | Investigation assignee |
...
| Systemic cause phase initiated | N-INV-SYSTEMIC | Safety manager (confirmation), division leadership |
...
- Stage 4 (`investigation.generate_narrative`) does not run — job is dropped, not queued
- Stage 5 (`fw_classify`) does not run — same
- Stage 3 (systemic cause phase) is blocked — `POST /api/v1/critical-insights` with this `trigger_id` is rejected
...
**Systemic cause phase pre-population (V2)**
In V1, the systemic cause phase requires the safety manager to author `pattern_summary`, `likely_systemic_cause`, and `recommended_action` manually. V2: pre-populate these fields from confirmed investigation framework fields — `root_cause` → `likely_systemic_cause`, top corrective action → `recommended_action`, AI-generated bridge summary → `pattern_summary`. Requires a new async job: `investigation.generate_systemic_bridge`.
...
*Last updated: May 2026. Update this file when: prompt text changes; framework field set changes; close gate rules change; systemic cause phase workflow changes; FW classification output schema changes. After updating, verify the prompt lab loads the updated text correctly and that `workflow-sim.html` scenario descriptions remain accurate. Check that `CRITICAL-INSIGHT.md` external_investigation trigger source documentation remains consistent with Stage 3 of this file.*
